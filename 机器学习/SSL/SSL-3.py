...
def read(d):
    ...
    X_1,X_test,y_1,y_test = train_test_split(X_0,y_0,test_size=0.2,random_state=15)
    X_2,X_3,y_2,y_3 = train_test_split(X_1,y_1,test_size=1-label_rate,random_state=17)
...
def preresults(cycle,):    
    pre_results['pre_1_'+str(cycle+1)]=None
    pre_results['pre_0_'+str(cycle+1)]=None
    count_1 = 0
    count_2 = 0
    for px in cats_added_positive.index:
        pre_results.loc[count_1,'pre_1_'+str(cycle+1)] = data_cats.iloc[px,len(data_cats.columns)-1]
        count_1 += 1
    for nx in cats_added_negative.index:
        pre_results.loc[count_2,'pre_0_'+str(cycle+1)] = data_cats.iloc[nx,len(data_cats.columns)-1]
        count_2 += 1
    prediction.loc[cycle+1] = (pre_results['pre_1_'+str(cycle+1)].sum()/addition + (1-pre_results['pre_0_'+str(cycle+1)].sum()/addition))/2       
...
if __name__ == '__main__':
    ...
    while cycle < cycles:
        ...
        cats_added_positive,cats_added_negative,cats_droplist = getindex(cats_chance)
        pre_results,prediction = preresults(cycle)
